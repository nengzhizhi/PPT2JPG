	...
	if (req.url == '/upload') {
		...
		async.waterfall([
			...
			//调用unoconv将ppt|pptx转成pdf
				spawnProcess("/usr/bin/unoconv", [filePath], function (err, code) {
					if (!!err || code != 0) {
				...
				spawnProcess("/usr/bin/convert", [pdfFilePath, jpgFilePath], function (err, code) {
					if (!!err || code != 0) {
				...
				async.during(
					function (cb) {
						imagePath = (page == 0) ? filePath + '.jpg' : filePath + '-' + page + '.jpg';
